/****** Object:  StoredProcedure [dbo].[load_NCSA_fact_P6_resource_spread_direct_validate]    Script Date: 6/12/2024 7:19:15 PM ******/
DROP PROCEDURE [dbo].[load_NCSA_fact_P6_resource_spread_direct_validate]
GO



CREATE PROC [dbo].[load_NCSA_fact_P6_resource_spread_direct_validate] AS
...
04/30/2024		Aditya			Data source has been updated to ProjectXD database View (p6_v), initially source used to be Excel Files.
...
**********************************************************/
	...
	update [gpd].[P6_resource_spread_direct]
	set load_date_time = @load_date_time;

--To get max date from period column
	declare @maxperiod varchar(40) 
	set @maxperiod =(select COALESCE(max(period),'1900-01-01') from[gpd].[P6_resource_spread_direct] where period!='dummmydate')
...
--To remove 'A' from Start_Date and format to datetime
	update [gpd].[P6_resource_spread_direct]
	set Start = format(cast(left(Start, 10) as SmallDateTime),'yyyy-MM-dd') from [gpd].[P6_resource_spread_direct]
	where Right(Start,1) = 'A'

--To remove 'A' from Finish_Date and format to datetime
	update [gpd].[P6_resource_spread_direct]
	set Finish = format(cast(left(Finish, 10) as SmallDateTime),'yyyy-MM-dd') from [gpd].[P6_resource_spread_direct]
	where Right(Finish,1) = 'A'
	
	
	update [gpd].[P6_resource_spread_direct]
	set t_spreadsheet_field =  REPLACE(reverse(SUBSTRING(REVERSE(Spreadsheet_Field),2,3)),'/','') from [gpd].[P6_resource_spread_direct]
	
	
	update [gpd].[P6_resource_spread_direct]
	set final_spreadsheet_field = 
								CASE 
									WHEN t_spreadsheet_field = 'CRE' THEN 'Cum Remaining Early'
									WHEN t_spreadsheet_field = 'CRL' THEN 'Cum Remaining Late'
									WHEN t_spreadsheet_field = 'RE' THEN 'Remaining Early'
									WHEN t_spreadsheet_field = 'RL' THEN 'Remaining Late'
									ELSE NULL
								END
	...
	set project_number = 
								case 
									when len(ltrim(rtrim(substring([Project], 1, charindex('-', [Project])-1))))= 6 
									then '99'+ltrim(rtrim(substring([Project], 1, charindex('-', [Project])-1))) 
									else ltrim(rtrim(substring([Project], 1, charindex('-', [Project])-1))) 
								end



	update [gpd].[P6_resource_spread_direct]
	set plantype_revision = REPLACE(CONCAT([Plan Type Description],'_',[Version No]),' ','') from [gpd].[P6_resource_spread_direct]
	
	update [gpd].[P6_resource_spread_direct]
	set data_sub_class = concat(plantype_revision,'_',final_spreadsheet_field) from [gpd].[P6_resource_spread_direct]

--Combine data from the Reprocess table for the current run

	insert into [gpd].[P6_resource_spread_direct]
	([project], [Activity_ID], [Activity_Name], [Original_Duration], [Actual_Duration], [Remaining_Duration], [Start], [Finish], [Actual_Start], [Actual_Finish], [Calendar], [Budgeted_Units], [Actual_Units], [Remaining_Units], [WBS], [WBS_Name], [Cost_Account], [Role],[Resource_ID],[Resource_ID_Name],[Resource_Name],[Resource_Type], [Change_Management], [Executed_By_Type], [Function], [Milestones], [Office], [Phase], [Region], [Schedule_Risk_Assessment], [Storage_Solutions], [Supply_Chain], [Prime], [SubPrime], [Spreadsheet_Field], [Period], [Units], [fx], [work_unit], [work_area], [cwp], [schedule_resource], [iwp], [pim], [roc], [data_class], [sub_source], [source], [unit_of_measure], [dim_entp_function_sk], [dim_entp_function_nk], [dim_entp_project_sk], [dim_entp_project_nk], [dim_entp_phase_sk], [dim_entp_phase_nk], [dim_entp_work_unit_sk], [dim_entp_work_unit_nk], [dim_entp_work_area_sk], [dim_entp_work_area_nk], [dim_entp_cwp_sk], [dim_entp_cwp_nk], [dim_entp_schedule_resource_sk], [dim_entp_schedule_resource_nk], [dim_entp_activity_resource_sk], [dim_entp_activity_resource_nk], [dim_entp_iwp_sk], [dim_entp_iwp_nk], [dim_entp_pim_sk], [dim_entp_pim_nk], [dim_entp_roc_sk], [dim_entp_roc_nk], [dim_entp_data_sub_class_sk], [dim_entp_data_sub_class_nk], [dim_entp_data_class_sk], [dim_entp_data_class_nk], [dim_entp_sub_source_sk], [dim_entp_sub_source_nk], [dim_entp_source_sk], [dim_entp_source_nk], [dim_entp_unit_of_measure_sk], [dim_entp_unit_of_measure_nk],[dim_entp_planning_resource_sk],[dim_entp_planning_resource_nk], [load_date_time], [parent_rssk], [hid], [Project_Description], [Total Float], [Free Float], [BL Project Start], [BL Project Finish], [Critical], [Activity Type], [Longest Path], [Primary Constraint Date], [Secondary Constraint Date], [Primary Constraint], [Secondary Constraint], [Expected Finish], [Suspend Date], [Resume Date], [BL Project Labor Units], [Earned Value Labor Units], [Planned Value Labor Units], [BL Project Duration], [Physical Percent Complete], [Predecessors], [Successors], [Predecessor Details], [Successors Details], [Late Start], [Late Finish], [t_spreadsheet_field], [final_spreadsheet_field], [project_number],[Plan Type],[Plan Type Description],[Version No], [plantype_revision], [data_sub_class], [created_date_time])
	SELECT 
	[project], [Activity_ID], [Activity_Name], [Original_Duration], [Actual_Duration], [Remaining_Duration], [Start], [Finish], [Actual_Start], [Actual_Finish], [Calendar], [Budgeted_Units], [Actual_Units], [Remaining_Units], [WBS], [WBS_Name], [Cost_Account], [Role],[Resource_ID],[Resource_ID_Name],[Resource_Name],[Resource_Type], [Change_Management], [Executed_By_Type], [Function], [Milestones], [Office], [Phase], [Region], [Schedule_Risk_Assessment], [Storage_Solutions], [Supply_Chain], [Prime], [SubPrime], [Spreadsheet_Field], [Period], [Units], [fx], [work_unit], [work_area], [cwp], [schedule_resource], [iwp], [pim], [roc], [data_class], [sub_source], [source], [unit_of_measure], [dim_entp_function_sk], [dim_entp_function_nk], [dim_entp_project_sk], [dim_entp_project_nk], [dim_entp_phase_sk], [dim_entp_phase_nk], [dim_entp_work_unit_sk], [dim_entp_work_unit_nk], [dim_entp_work_area_sk], [dim_entp_work_area_nk], [dim_entp_cwp_sk], [dim_entp_cwp_nk], [dim_entp_schedule_resource_sk], [dim_entp_schedule_resource_nk], [dim_entp_activity_resource_sk], [dim_entp_activity_resource_nk], [dim_entp_iwp_sk], [dim_entp_iwp_nk], [dim_entp_pim_sk], [dim_entp_pim_nk], [dim_entp_roc_sk], [dim_entp_roc_nk], [dim_entp_data_sub_class_sk], [dim_entp_data_sub_class_nk], [dim_entp_data_class_sk], [dim_entp_data_class_nk], [dim_entp_sub_source_sk], [dim_entp_sub_source_nk], [dim_entp_source_sk], [dim_entp_source_nk], [dim_entp_unit_of_measure_sk], [dim_entp_unit_of_measure_nk],[dim_entp_planning_resource_sk],[dim_entp_planning_resource_nk], [load_date_time], ISNULL([parent_rssk],[rssk]), [hid], [Project_Description], [Total Float], [Free Float], [BL Project Start], [BL Project Finish], [Critical], [Activity Type], [Longest Path], [Primary Constraint Date], [Secondary Constraint Date], [Primary Constraint], [Secondary Constraint], [Expected Finish], [Suspend Date], [Resume Date], [BL Project Labor Units], [Earned Value Labor Units], [Planned Value Labor Units], [BL Project Duration], [Physical Percent Complete], [Predecessors], [Successors], [Predecessor Details], [Successors Details], [Late Start], [Late Finish], [t_spreadsheet_field], [final_spreadsheet_field], [project_number],[Plan Type],[Plan Type Description],[Version No], [plantype_revision], [data_sub_class], [created_date_time]
	
    from gpd.reprocess_P6_resource_spread_direct

	delete from gpd.reprocess_P6_resource_spread_direct
	...
	--set plantype_revision=SUBSTRING(Spreadsheet_Field, 1, CHARINDEX('_', Spreadsheet_Field)+1 )
       ...
       'gpd.P6_resource_spread_direct','gpd.P6_resource_spread_direct','stage_p6_engr_id', 
       'dim_entp_project_sk','dim_entp_project_nk','dim_entp_project','dim_entp_project_sk','dim_entp_project_nk', 
       'project_number','project_number','FL',null;
	   
	   	   
truncate table pxd.stage_projects

insert into pxd.stage_projects
( [description],[area_code],contract_type_lov_code,[project_no],[customer])
SELECT DISTINCT Project_Description,'unknown_','Unknown_project', project_number,'Unknown_project' FROM
gpd.P6_resource_spread_direct where dim_entp_project_nk is null

execute [dbo].[generic_load_dim_v2 ] 'dim_ent_project_xd','Y','N','Y'
...
truncate table [gpd].[NCSA_planning_resource]
		...
		INSERT INTO [gpd].[NCSA_planning_resource]
           ...
           ,[project])
			...
			from (select distinct 
					...
					[Resource_Type],
					[project_number]
					from [gpd].[P6_resource_spread_direct]) x
			group by x.[Resource_ID],x.[project_number]


			exec generic_load_dim_v2 'dim_entp_planning_resource', 'Y', 'N', 'Y'

			execute [stage].[fact_lookup_based_on_many_key]
        'gpd.P6_resource_spread_direct', 'gpd.P6_resource_spread_direct',  'stage_p6_engr_id', 'dim_entp_planning_resource_sk', 'dim_entp_planning_resource_nk',
       ...
       'fx','function_code','FL',null;
		...
		execute [stage].[fact_lookup_based_on_many_key]
        'gpd.P6_resource_spread_direct', 'gpd.P6_resource_spread_direct',  'stage_p6_engr_id', 'dim_entp_phase_sk', 'dim_entp_phase_nk',
		...
		execute [stage].[fact_lookup_based_on_many_key]
        'gpd.P6_resource_spread_direct', 'gpd.P6_resource_spread_direct',  'stage_p6_engr_id', 'dim_entp_work_unit_sk', 'dim_entp_work_unit_nk',
		...
		execute [stage].[fact_lookup_based_on_many_key]
        'gpd.P6_resource_spread_direct', 'gpd.P6_resource_spread_direct',  'stage_p6_engr_id', 'dim_entp_work_area_sk', 'dim_entp_work_area_nk',
		...
		execute [stage].[fact_lookup_based_on_many_key]
        'gpd.P6_resource_spread_direct', 'gpd.P6_resource_spread_direct',  'stage_p6_engr_id', 'dim_entp_cwp_sk', 'dim_entp_cwp_nk',
		...
		execute [stage].[fact_lookup_based_on_many_key]
        'gpd.P6_resource_spread_direct', 'gpd.P6_resource_spread_direct',  'stage_p6_engr_id', 'dim_entp_schedule_resource_sk', 'dim_entp_schedule_resource_nk',
		...
		execute [stage].[fact_lookup_based_on_many_key]
        'gpd.P6_resource_spread_direct', 'gpd.P6_resource_spread_direct',  'stage_p6_engr_id', 'dim_entp_iwp_sk', 'dim_entp_iwp_nk',
		...
		execute [stage].[fact_lookup_based_on_many_key]
        'gpd.P6_resource_spread_direct', 'gpd.P6_resource_spread_direct',  'stage_p6_engr_id', 'dim_entp_pim_sk', 'dim_entp_pim_nk',
		...
		execute [stage].[fact_lookup_based_on_many_key]
        'gpd.P6_resource_spread_direct', 'gpd.P6_resource_spread_direct',  'stage_p6_engr_id', 'dim_entp_roc_sk', 'dim_entp_roc_nk',
...
insert into [gpd].[tmp_Activity_P6_engr_headcount]
		...
		select
		  ...
		  from 
			...
			P.schedule_resource,
			P.Original_Duration,
			P.Remaining_Duration as remaining_duration,
			P.Actual_Start as actual_start,
			P.Actual_Finish as actual_finish,
			A.percent_complete_type,		
			P.Executed_By_Type as executed_by_type,
			P.prime,
			...
			A.early_start_date,		
			A.early_finish_date,	
			P.Critical as is_on_critical_path,
			P.[Total Float] as total_float,
			A.activity_status,
			...
			P.Actual_Duration as actual_duration,
			A.remaining_late_start,
			A.remaining_late_finish,
			P.Calendar as calendar,
			P.Budgeted_Units as budgeted_units,
			P.Actual_Units as actual_units,
			P.Remaining_Units as remaining_units,
			P.WBS as wbs,
			P.WBS_Name as wbs_name,
			P.Cost_Account as cost_account,
			P.[Role] as role_is,
			P.Change_Management as change_management,
			P.[Function] as funct,
			P.Milestones as milestones,
			P.Office as office,
			A.dim_entp_phase_nk,
			P.Region as region,
			P.Schedule_Risk_Assessment as schedule_risk_assessment,
			P.Supply_Chain as supply_chain
			from
			[gpd].[P6_resource_spread_direct] P
			left join [dbo].[dim_entp_activity] A
			on P.Activity_ID = A.activity_code
			and P.project_number=A.project_number
			and A.active=1
			WHERE P.final_spreadsheet_field like '%Cum Remaining Early%'   
			and P.dim_entp_project_nk is not null
			) AA
			group by 
			[activity_code]
		  ,[activity_description]
...
insert into [gpd].[tmp_P6_resource_activity]
		...
		select distinct
		  ...
		  ,max([calendar]) as [calendar]
		  ,max([resource_id_name]) as [resource_id_name]
		  ,max([resource_name]) as [resource_name]
		  ,max([resource_type]) as [resource_type]
		  ,max([phase]) as [phase]
		  ,max([storage_solutions]) as [storage_solutions]
		  ,max([supply_chain]) as [supply_chain]
		  ,max([spreadsheet_field]) as [spreadsheet_field]
		  ,max([work_unit]) as [work_unit]
		  ,max([iwp]) as [iwp]
		  ,max([pim]) as [pim]
		  ,max([roc]) as [roc]
		  ,max([data_class]) as [data_class]
		  ,max([sub_source]) as [sub_source]
		  ,max([source]) as [source]
		  ,max([unit_of_measure]) as [unit_of_measure]
		  ,max([free_float]) as [free_float]
		  ,max([bl_project_start]) as [bl_project_start]
		  ,max([bl_project_finish]) as [bl_project_finish]
		  ,max([activity_type]) as [activity_type]
		  ,max([longest_path]) as [longest_path]
		  ,max([primary_constraint_date])  as [primary_constraint_date]
		  ,max([secondary_constraint_date])  as [secondary_constraint_date]
		  ,max([primary_constraint])  as [primary_constraint]
		  ,max([secondary_constraint])  as [secondary_constraint]
		  ,max([expected_finish]) as [expected_finish]
		  ,max([suspend_date]) as [suspend_date]
		  ,max([resume_date])  as [resume_date]
		  ,max([bl_project_labor_units]) as [bl_project_labor_units]
		  ,max([earned_value_labor_units]) as [earned_value_labor_units]
		  ,max([planned_value_labor_units]) as [planned_value_labor_units]
		  ,max([bl_project_duration]) as [bl_project_duration]
		  ,max([physical_percent_complete]) as [physical_percent_complete]
		  ,max([predecessors]) as [predecessors]
		  ,max([successors]) as [successors]
		  ,max([predecessor_details])  as [predecessor_details]
		  ,max([successors_details]) as [successors_details]
		  ,max([late_start])  as [late_start]
		  ,max([late_finish]) as [late_finish]
		  ...
		  from 
			...
			,P.[source]
			,P.[unit_of_measure]
			,P.[Free Float] as free_float
			,P.[BL Project Start] as bl_project_start
			,P.[BL Project Finish] as bl_project_finish
			,P.[Activity Type] as activity_type
			,P.[Longest Path] as  longest_path
			,P.[Primary Constraint Date] as  primary_constraint_date
			,P.[Secondary Constraint Date] as  secondary_constraint_date
			,P.[Primary Constraint] as primary_constraint 
			,P.[Secondary Constraint] as  secondary_constraint
			,P.[Expected Finish] as expected_finish 
			,P.[Suspend Date] as  suspend_date
			,P.[Resume Date] as resume_date 
			,P.[BL Project Labor Units]  as  bl_project_labor_units
			,P.[Earned Value Labor Units] as  earned_value_labor_units
			,P.[Planned Value Labor Units] as  planned_value_labor_units
			,P.[BL Project Duration] as  bl_project_duration
			,P.[Physical Percent Complete] as  physical_percent_complete
			,P.[Predecessors] as  predecessors
			,P.[Successors] as successors
			,P.[Predecessor Details] as  predecessor_details
			,P.[Successors Details] as successors_details 
			,P.[Late Start] as  late_start
			,P.[Late Finish] as  late_finish
...
exec generic_load_dim_v2 'dim_entp_p6_resource_activity', 'Y', 'N', 'Y'
		...
		/*
		execute [stage].[fact_lookup_based_on_many_key]
        'gpd.P6_resource_spread_direct', 'gpd.P6_resource_spread_direct',  'stage_p6_engr_id', 'dim_entp_activity_resource_sk', 'dim_entp_activity_resource_nk',
		...
		*/
		...
		from [gpd].[P6_resource_spread_direct] a,dbo.dim_entp_activity_resource b
		where a.dim_entp_planning_resource_nk = b.dim_entp_planning_resource_nk
		and a.plantype_revision = b.plantype_revision
...
--04/21/2022 : Clear the stage table of all dummy records
		...
		--select * from [gpd].[P6_resource_spread_direct]
		DELETE FROM [gpd].[P6_resource_spread_direct]
		where period='dummmydate' and units='0'
		...
		select data_class,data_sub_class into #tmp_sub_class 
		from (select distinct data_class,data_sub_class from [gpd].[P6_resource_spread_direct] where final_spreadsheet_field <>'' )p;
       ...
       'data_class','data_class_name','FL',null;
		...
		execute [stage].[fact_lookup_based_on_many_key]
        'gpd.P6_resource_spread_direct', 'gpd.P6_resource_spread_direct',  'stage_p6_engr_id', 'dim_entp_data_sub_class_sk', 'dim_entp_data_sub_class_nk',
        'dim_ent_data_sub_class', 'dim_ent_data_sub_class_sk', 'dim_ent_data_sub_class_nk', 'dim_entp_data_class_nk', 'dim_ent_data_class_nk',
		'data_sub_class', 'data_sub_class_name',NULL,NULL,NULL,NULL,NULL,NULL,'FL',NULL ;
       ...
       'source','source_name','FL',null;
		...
		execute [stage].[fact_lookup_based_on_many_key]
        'gpd.P6_resource_spread_direct', 'gpd.P6_resource_spread_direct',  'stage_p6_engr_id', 'dim_entp_sub_source_sk', 'dim_entp_sub_source_nk',
	   ...
	   'dim_ent_unit_of_measure_nk', 'unit_of_measure','unit_of_measure_code','L',null;
		...
		select @unprocessed_count  = count(*)  from [gpd].[P6_resource_spread_direct]
		where 
		...
		select @processed_count  = count(*)  from [gpd].[P6_resource_spread_direct]
		where 
		...
		insert into [gpd].[reprocess_P6_resource_spread_direct]
		([project], [Activity_ID], [Activity_Name], [Original_Duration], [Actual_Duration], [Remaining_Duration], [Start], [Finish], [Actual_Start], [Actual_Finish],[Calendar], [Budgeted_Units], [Actual_Units], [Remaining_Units], [WBS], [WBS_Name], [Cost_Account], [Role],[Resource_ID],[Resource_ID_Name],[Resource_Name],[Resource_Type], [Change_Management], [Executed_By_Type], [Function], [Milestones], [Office], [Phase], [Region], [Schedule_Risk_Assessment], [Storage_Solutions], [Supply_Chain], [Prime], [SubPrime], [Spreadsheet_Field], [Period], [Units], [fx], [work_unit], [work_area], [cwp], [schedule_resource], [iwp], [pim], [roc], [data_class], [sub_source], [source], [unit_of_measure], [dim_entp_function_sk], [dim_entp_function_nk], [dim_entp_project_sk], [dim_entp_project_nk], [dim_entp_phase_sk], [dim_entp_phase_nk], [dim_entp_work_unit_sk], [dim_entp_work_unit_nk], [dim_entp_work_area_sk], [dim_entp_work_area_nk], [dim_entp_cwp_sk], [dim_entp_cwp_nk], [dim_entp_schedule_resource_sk], [dim_entp_schedule_resource_nk], [dim_entp_activity_resource_sk], [dim_entp_activity_resource_nk], [dim_entp_iwp_sk], [dim_entp_iwp_nk], [dim_entp_pim_sk], [dim_entp_pim_nk], [dim_entp_roc_sk], [dim_entp_roc_nk], [dim_entp_data_sub_class_sk], [dim_entp_data_sub_class_nk], [dim_entp_data_class_sk], [dim_entp_data_class_nk], [dim_entp_sub_source_sk], [dim_entp_sub_source_nk], [dim_entp_source_sk], [dim_entp_source_nk], [dim_entp_unit_of_measure_sk], [dim_entp_unit_of_measure_nk],[dim_entp_planning_resource_sk],[dim_entp_planning_resource_nk], [load_date_time], [parent_rssk], [hid], [Project_Description], [Total Float], [Free Float], [BL Project Start], [BL Project Finish], [Critical], [Activity Type], [Longest Path], [Primary Constraint Date], [Secondary Constraint Date], [Primary Constraint], [Secondary Constraint], [Expected Finish], [Suspend Date], [Resume Date], [BL Project Labor Units], [Earned Value Labor Units], [Planned Value Labor Units], [BL Project Duration], [Physical Percent Complete], [Predecessors], [Successors], [Predecessor Details], [Successors Details], [Late Start], [Late Finish], [t_spreadsheet_field], [final_spreadsheet_field], [project_number],[Plan Type],[Plan Type Description],[Version No], [plantype_revision], [data_sub_class], [created_date_time])
		select 
		[project], [Activity_ID], [Activity_Name], [Original_Duration], [Actual_Duration], [Remaining_Duration], [Start], [Finish], [Actual_Start], [Actual_Finish], [Calendar], [Budgeted_Units], [Actual_Units], [Remaining_Units], [WBS], [WBS_Name], [Cost_Account], [Role],[Resource_ID],[Resource_ID_Name],[Resource_Name],[Resource_Type], [Change_Management], [Executed_By_Type], [Function], [Milestones], [Office], [Phase], [Region], [Schedule_Risk_Assessment], [Storage_Solutions], [Supply_Chain], [Prime], [SubPrime], [Spreadsheet_Field], [Period], [Units], [fx], [work_unit], [work_area], [cwp], [schedule_resource], [iwp], [pim], [roc], [data_class], [sub_source], [source], [unit_of_measure], [dim_entp_function_sk], [dim_entp_function_nk], [dim_entp_project_sk], [dim_entp_project_nk], [dim_entp_phase_sk], [dim_entp_phase_nk], [dim_entp_work_unit_sk], [dim_entp_work_unit_nk], [dim_entp_work_area_sk], [dim_entp_work_area_nk], [dim_entp_cwp_sk], [dim_entp_cwp_nk], [dim_entp_schedule_resource_sk], [dim_entp_schedule_resource_nk], [dim_entp_activity_resource_sk], [dim_entp_activity_resource_nk], [dim_entp_iwp_sk], [dim_entp_iwp_nk], [dim_entp_pim_sk], [dim_entp_pim_nk], [dim_entp_roc_sk], [dim_entp_roc_nk], [dim_entp_data_sub_class_sk], [dim_entp_data_sub_class_nk], [dim_entp_data_class_sk], [dim_entp_data_class_nk], [dim_entp_sub_source_sk], [dim_entp_sub_source_nk], [dim_entp_source_sk], [dim_entp_source_nk], [dim_entp_unit_of_measure_sk], [dim_entp_unit_of_measure_nk],[dim_entp_planning_resource_sk],[dim_entp_planning_resource_nk], [load_date_time], [parent_rssk], [hid], [Project_Description], [Total Float], [Free Float], [BL Project Start], [BL Project Finish], [Critical], [Activity Type], [Longest Path], [Primary Constraint Date], [Secondary Constraint Date], [Primary Constraint], [Secondary Constraint], [Expected Finish], [Suspend Date], [Resume Date], [BL Project Labor Units], [Earned Value Labor Units], [Planned Value Labor Units], [BL Project Duration], [Physical Percent Complete], [Predecessors], [Successors], [Predecessor Details], [Successors Details], [Late Start], [Late Finish], [t_spreadsheet_field], [final_spreadsheet_field], [project_number],[Plan Type],[Plan Type Description],[Version No], [plantype_revision], [data_sub_class], [created_date_time]
		
		from [gpd].[P6_resource_spread_direct]
		where 
		   dim_entp_function_sk is null
		or dim_entp_function_nk is null
		or dim_entp_project_sk is null
	...
	--------------------------------------------------------------------------------------
		...
		update [gpd].[reprocess_P6_resource_spread_direct]
		 set rk_failure_info = 
		 case when dim_entp_function_nk is null then 'Missing Function - ' + isnull(fx,'') + ',' else '' end +
		 case when dim_entp_project_nk is null then 'Missing Project Number - ' + isnull(project_number,'') + ',' else '' end +
		 case when dim_entp_data_class_nk is null then 'Missing Data Class - ' + isnull(data_class,'') + ',' else '' end +
		 case when dim_entp_data_sub_class_nk is null then 'Missing Data Sub Class - ' + isnull(data_sub_class,'') + ',' else '' end +
		 ...
		 case when dim_entp_activity_resource_nk is null then 'Missing Activity Code - ' +  + isnull(activity_id,'') + ',' + 'Missing Resource Id - ' +  + isnull(resource_id,'') + ','  + 'Missing Project Number - ' +  + isnull(project_number,'') + ',' else '' end +
...
---------------------------------------------------------------------------
	...
	exec populate_reprocess_history 'pl_resource_spread_direct_master_pipeline', 'gpd.P6_resource_spread_direct', 'gpd.reprocess_P6_resource_spread_direct', 'dbo.fact_ops_planning','rssk';
