/****** Object:  View [dbo].[fact_pmr_MCPM_Offshore_v]    Script Date: 5/22/2022 8:45:32 PM ******/
SET ANSI_NULLS ON
GO
...
,[created_date_time]
,isnull([transaction_quantity],0) [transaction_quantity]
				from --[dbo].[fact_ops]  --08082022 Temp change to handle the 'Psuedo duplicate' records in FACT
					( select MAX(fact_entp_sk) as fact_entp_sk,
							[dim_entp_function_nk] ,[dim_entp_project_nk] ,[dim_entp_phase_nk] ,[dim_entp_work_unit_nk] ,[dim_entp_work_area_nk] ,[dim_entp_cwp_nk] ,dim_entp_schedule_resource_nk ,[dim_entp_activity_nk] ,[dim_entp_iwp_nk] ,[dim_entp_pim_nk] ,[dim_entp_roc_nk] ,[dim_entp_data_sub_class_nk] ,dim_entp_data_class_nk ,[dim_entp_sub_source_nk] ,dim_entp_source_nk ,[dim_entp_unit_of_measure_nk] ,[dim_ent_employee_nk] ,dim_ent_jde_coa_nk  ,dim_ent_geac_coa_nk ,[transaction_date] ,[created_date_time]
							...
							GROUP BY
							[dim_entp_function_nk] ,[dim_entp_project_nk] ,[dim_entp_phase_nk] ,[dim_entp_work_unit_nk] ,[dim_entp_work_area_nk] ,[dim_entp_cwp_nk] ,dim_entp_schedule_resource_nk ,[dim_entp_activity_nk] ,[dim_entp_iwp_nk] ,[dim_entp_pim_nk] ,[dim_entp_roc_nk] ,[dim_entp_data_sub_class_nk] ,dim_entp_data_class_nk ,[dim_entp_sub_source_nk] ,dim_entp_source_nk ,[dim_entp_unit_of_measure_nk] ,[dim_ent_employee_nk] ,dim_ent_jde_coa_nk  ,dim_ent_geac_coa_nk ,[transaction_date] ,[created_date_time]
				    ) Intr   --08082022 Temp change to handle the 'Psuedo duplicate' records in FACT. Eventually to be removed from the view.
				) a 
				where 
				dim_entp_sub_source_nk in (select dim_ent_sub_source_nk from [dbo].[dim_ent_sub_source] where sub_source_name in ('MCPM Offshore'))
				AND 
...
-----------Onshore_view---
/****** Object:  View [dbo].[fact_pmr_MCPM_Onshore_v]    Script Date: 5/22/2022 8:45:17 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [dbo].[fact_pmr_MCPM_Onshore_v]
AS select z.*,f.dim_ent_prime_nk from 
(select 
fact_entp_sk	  
,[dim_entp_function_nk]
