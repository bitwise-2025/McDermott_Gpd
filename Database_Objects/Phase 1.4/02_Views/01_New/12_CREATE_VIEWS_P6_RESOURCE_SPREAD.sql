...
where active = 1;
GO
/****** Object:  View [dbo].[fact_pmr_p6_v]    Script Date: 8/5/2022 3:14:49 AM ******/
IF	OBJECT_ID('[dbo].[fact_pmr_p6_v]','V') IS NOT NULL
DROP VIEW [dbo].[fact_pmr_p6_v]
GO
/****** Object:  View [dbo].[fact_pmr_p6_v]    Script Date: 8/5/2022 3:14:49 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [dbo].[fact_pmr_p6_v]
AS select z.*,oa.dim_ent_office_nk, DATEADD(day, ISNULL(CP.days_diff,0), z.transaction_date_original ) as transaction_date from (
select * from 
/* VERSION TRACKER
...
(select 
				...
				WHERE dim_entp_sub_source_nk in (select dim_ent_sub_source_nk from [dbo].[dim_ent_sub_source] where sub_source_name in ('P6-Direct')) 
				...
				LEFT OUTER JOIN [dbo].[dim_entp_project_week_closing_period] PWC ON z.[dim_entp_project_nk]=PWC.[dim_entp_project_nk]  --08/05/2022	Satish
