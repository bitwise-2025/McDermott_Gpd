...
select ROW_NUMBER() OVER (ORDER BY ro_category) as stage_promt_risk_opportunity_id,ro_category,ro_category_description,sub_category from (
select distinct ro_category,ro_category_description,sub_category  
from [promt].[reprocess_promt_datalake_export_risk_snapshot] )a

UNION ALL

select ROW_NUMBER() OVER (ORDER BY ro_category) as stage_promt_risk_opportunity_id,ro_category,ro_category_description,sub_category from (
select distinct ro_category,ro_category_description,sub_category  
from [promt].[reprocess_promt_datalake_export_opportunity_snapshot] )a
...
ALTER View [promt].[stage_promt_cost_impact_tcv_percentage_v] as
...
Alter View [promt].[stage_promt_overall_impact_v] as
...
ALTER View [promt].[stage_promt_mit_environment_v] as
...
ALTER View [promt].[stage_promt_mit_health_safety_security_v] as
...
ALTER View [promt].[stage_promt_mit_quality_v] as
...
ALTER View [promt].[stage_promt_mit_reputation_v] as
...
ALTER View [promt].[stage_promt_mit_social_culture_v] as
...
ALTER View [promt].[stage_promt_mit_cost_impact_tcv_percentage_v] as
...
ALTER View [promt].[stage_promt_mit_overall_impact_v] as
