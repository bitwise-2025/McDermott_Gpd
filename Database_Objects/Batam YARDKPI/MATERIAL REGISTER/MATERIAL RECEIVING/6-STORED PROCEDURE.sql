...
CREATE PROC [dbo].[usp_yardKPI_material_receiving_dim_fact_load_validate] AS
...
begin
	begin try
		...
		set @processed_count=(select count(*) from yardkpi.stage_material_Receiving_register)
		...
		if @processed_count > 0 
			begin 
				...
				EXECUTE [dbo].[generic_load_dim_v2] 'dim_entp_yardkpi_mat_recv_project','Y','N','Y'

				EXECUTE [dbo].[generic_load_dim_v2] 'dim_entp_yardkpi_matrecvcontract','Y','N','Y'

				EXECUTE [dbo].[generic_load_dim_v2] 'dim_entp_yardkpi_material_receiving','Y','N','Y'
