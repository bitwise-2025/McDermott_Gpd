...
CREATE PROC [dbo].[usp_yardkpi_mwin_dim_load] AS
...
begin
	begin try
		...
		set @processed_count=(select count(*) from [yardkpi].[stage_entp_mwin_rfi_milestone])
		...
		if @processed_count > 0 
			begin 
				...
				EXECUTE [dbo].[generic_load_dim_v2] 'dim_entp_mwin_project','Y','N','Y'

				EXECUTE [dbo].[generic_load_dim_v2] 'dim_ent_mwin_contract','Y','N','Y'

				EXECUTE [dbo].[generic_load_dim_v2] 'dim_entp_mwin_rfi_milestone','Y','N','Y'
			end
		else
			begin 
