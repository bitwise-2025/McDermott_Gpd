CREATE PROC [dbo].[usp_fact_fss_revenue_load] AS
BEGIN
   DECLARE  @Errormessage varchar(max)
   DECLARE  @datetimekey datetime

declare @reportingperiod_revenue date
		select @datetimekey=max(zCreatedDateTime) from fss.stage_fss_revenue_v
if exists (select 1 from fss.Stage_fss_reportingperiod_V where  [Data source]='Revenue' and month([Reporting Period]) not between 0 and 12)
    BEGIN
        set @Errormessage='Invalid Month Range'
        select @Errormessage
    End

set @reportingperiod_revenue =(select distinct [Reporting Period] from fss.Stage_fss_reportingperiod_V where [Data source]='Revenue')


if (month(@reportingperiod_revenue) between 1 and 3)
    ...
    begin 
      ...
      ,[Quarter],[Month],[Amount],[created_date_time] )
	    select 
c.dim_ent_source_nk,c.dim_ent_sub_source_nk,c.dim_entp_fss_project_nk,d.dim_ent_data_class_nk,d.dim_ent_data_sub_class_nk,v.dim_entp_fss_business_line_nk,'Revenue' as [Data Source]
,[New/ CO],[JV/Non-JV],null as BLPW,[External Product Line],[Customer],null as [Seq No],null as [Sub Title],null as [Caclulation],
null as [Category],null as [Facility],null as [Commentary],@reportingperiod_revenue as TransactionDate,'Q1'  AS [Quarter],
...
,[Total  Prev Qtr] ,[Total  Delta Prev Qtr] ,[Total  Current Qtr] ,[Delta Current Qtr] ,[Jan F2] ,[Feb F2] ,[Mar F2] ,[Total  F2] ,[Jan Delta F2] ,[Feb Delta F2] ,[Mar Delta F2] ,[Delta F2]
                ...
                on b.[Project No.]=c.[Project_No] and trim(b.[Project Title])=trim(c.[Project_Title])
...
if (month(@reportingperiod_revenue) between 4 and 6)
...
c.dim_ent_source_nk,c.dim_ent_sub_source_nk,c.dim_entp_fss_project_nk,d.dim_ent_data_class_nk,d.dim_ent_data_sub_class_nk,v.dim_entp_fss_business_line_nk,'Revenue' as [Data Source]
,[New/ CO],[JV/Non-JV],null as BLPW,[External Product Line],[Customer],null as [Seq No],null as [Sub Title],null as [Caclulation],
null as [Category],null as [Facility],null as [Commentary],@reportingperiod_revenue as TransactionDate,'Q2'  AS [Quarter],
...
if (month(@reportingperiod_revenue) between 7 and 9)
...
c.dim_ent_source_nk,c.dim_ent_sub_source_nk,c.dim_entp_fss_project_nk,d.dim_ent_data_class_nk,d.dim_ent_data_sub_class_nk,v.dim_entp_fss_business_line_nk,'Revenue' as [Data Source]
,[New/ CO],[JV/Non-JV],null as BLPW,[External Product Line],[Customer],null as [Seq No],null as [Sub Title],null as [Caclulation],
null as [Category],null as [Facility],null as [Commentary],@reportingperiod_revenue as TransactionDate,'Q3'  AS [Quarter],
...
if (month(@reportingperiod_revenue) between 10 and 12)
...
c.dim_ent_source_nk,c.dim_ent_sub_source_nk,c.dim_entp_fss_project_nk,d.dim_ent_data_class_nk,d.dim_ent_data_sub_class_nk,v.dim_entp_fss_business_line_nk,'Revenue' as [Data Source]
,[New/ CO],[JV/Non-JV],null as BLPW,[External Product Line],[Customer],null as [Seq No],null as [Sub Title],null as [Caclulation],
null as [Category],null as [Facility],null as [Commentary],@reportingperiod_revenue as TransactionDate,'Q4'  AS [Quarter],
