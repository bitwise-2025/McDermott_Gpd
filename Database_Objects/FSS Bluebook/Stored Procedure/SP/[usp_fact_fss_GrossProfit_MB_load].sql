CREATE PROC [dbo].[usp_fact_fss_GrossProfit_MB_load] AS
BEGIN
   DECLARE  @Errormessage varchar(max)
   DECLARE  @datetimekey datetime

  declare @reportingperiod_GrossProfit_mb date

		select @datetimekey=max(zCreatedDateTime) from fss.[stage_fss_gross_profit_mb_v]

if exists (select * from fss.Stage_fss_reportingperiod_V where  [Data source]='Gross Profit MB' and month([Reporting Period]) not between 0 and 12)
    BEGIN
        set @Errormessage='Invalid Month Range'
        select @Errormessage
    End

set @reportingperiod_GrossProfit_mb =(select distinct [Reporting Period] from fss.Stage_fss_reportingperiod_V where [Data source]='Gross Profit MB')
...
'Gross Profit MB' as [Data Source],null as [New/ CO],[JV/Non-JV],[BL/PW] as BLPW,[External Product Line],[Customer],null as [Seq No],null as [Sub Title],null as [Caclulation],
null as [Category],null as [Facility],null as [Commentary]
,@reportingperiod_GrossProfit_mb as TransactionDate
,null AS [Quarter],
...
--,isnull([X50],'') as [X50]
 ...
 from [fss].[stage_fss_gross_profit_mb_v]
