CREATE PROC [dbo].[usp_fact_fss_Revenue_MB_load] AS
BEGIN
...
if exists (select * from fss.Stage_fss_reportingperiod_V where  [Data source]='Revenue MB' and month([Reporting Period]) not between 0 and 12)
    BEGIN
        set @Errormessage='Invalid Month Range'
        select @Errormessage
    End

set @reportingperiod_revenue_mb =(select distinct [Reporting Period] from fss.Stage_fss_reportingperiod_V where [Data source]='Revenue MB')
