--***** Object:  View [gpd].[Marine_campaign_readiness_PMR_JSON_active_V]    Script Date: 10/10/2022 10:38:08 PM *****
IF	OBJECT_ID('[gpd].[Marine_campaign_readiness_PMR_JSON_active_V]','V') IS NOT NULL
DROP VIEW [gpd].[Marine_campaign_readiness_PMR_JSON_active_V]
GO
--****** Object:  View [gpd].[Marine_Progress_PMR_JSON_active_V]    Script Date: 10/10/2022 10:38:08 PM *****
IF	OBJECT_ID('[gpd].[Marine_Progress_PMR_JSON_active_V]','V') IS NOT NULL
DROP VIEW [gpd].[Marine_Progress_PMR_JSON_active_V]
GO
--****** Object:  View [gpd].[ScheduleAnalytics_PMR_JSON_active_v]    Script Date: 10/10/2022 10:38:08 PM *****
IF	OBJECT_ID('[gpd].[ScheduleAnalytics_PMR_JSON_active_v]','V') IS NOT NULL
DROP VIEW [gpd].[ScheduleAnalytics_PMR_JSON_active_v]
GO
--****** Object:  View [gpd].[Marine_SpreadRate_PMR_JSON_active_V]    Script Date: 10/10/2022 10:38:08 PM *****
IF	OBJECT_ID('[gpd].[Marine_SpreadRate_PMR_JSON_active_V]','V') IS NOT NULL
DROP VIEW [gpd].[Marine_SpreadRate_PMR_JSON_active_V]
GO



CREATE VIEW [gpd].[Marine_campaign_readiness_PMR_JSON_active_V]
AS SELECT  p.dim_entp_project_nk,c.dim_ent_contract_nk,v.* FROM [gpd].[Marine_campaign_readiness_PMR_JSON_V] v
LEFT JOIN dim_entp_project_active_v p on TRIM(v.[project number])=p.project_number  --GET project_NK
LEFT JOIN dim_ent_contract_active_v c on TRIM(v.[contract number])=c.contract_number  --GET Contract_NK
WHERE [Project Number] + [Contract Number] + [Date] + zVersionNo in (
SELECT [Project Number] + [Contract Number] + [Date] + CONVERT(VARCHAR,(MAX(CAST(zVersionNo AS INT)))) FROM [gpd].[Marine_campaign_readiness_PMR_JSON_V]
GROUP BY [Project Number] + [Contract Number] + [Date]);
GO


CREATE VIEW [gpd].[Marine_Progress_PMR_JSON_active_V]
AS SELECT  p.dim_entp_project_nk,c.dim_ent_contract_nk,v.* FROM [gpd].[Marine_Progress_PMR_JSON_V] v
LEFT JOIN dim_entp_project_active_v p on TRIM(v.[project number])=p.project_number  --GET project_NK
LEFT JOIN dim_ent_contract_active_v c on TRIM(v.[contract number])=c.contract_number  --GET Contract_NK
WHERE [Project Number] + [Contract Number] + [Date] + zVersionNo in (
SELECT [Project Number] + [Contract Number] + [Date] + CONVERT(VARCHAR,(MAX(CAST(zVersionNo AS INT)))) FROM [gpd].[Marine_Progress_PMR_JSON_V] 
GROUP BY [Project Number] , [Contract Number] , [Date]);
GO


CREATE VIEW [gpd].[ScheduleAnalytics_PMR_JSON_active_v]
AS SELECT p.dim_entp_project_nk,c.dim_ent_contract_nk,v.* FROM gpd.ScheduleAnalytics_PMR_JSON_V v
LEFT JOIN dim_entp_project_active_v p on TRIM(v.[project number])=p.project_number  --GET project_NK
LEFT JOIN dim_ent_contract_active_v c on TRIM(v.[contract number])=c.contract_number  --GET Contract_NK
WHERE [Project Number] + [Contract Number] + [Start] + zVersionNo in (
SELECT [Project Number] + [Contract Number] + [Start] + CONVERT(VARCHAR,(MAX(CAST(zVersionNo AS INT)))) FROM gpd.ScheduleAnalytics_PMR_JSON_V
GROUP BY [Project Number] + [Contract Number] + [Start]);
GO

CREATE VIEW [gpd].[Marine_SpreadRate_PMR_JSON_active_V]
AS SELECT  p.dim_entp_project_nk,c.dim_ent_contract_nk,v.* FROM [gpd].[Marine_Spreadrate_PMR_JSON_v] v
LEFT JOIN dim_entp_project_active_v p on TRIM(v.[project number])=p.project_number  --GET project_NK
LEFT JOIN dim_ent_contract_active_v c on TRIM(v.[contract number])=c.contract_number  --GET Contract_NK
WHERE [Project Number] + [Contract Number] + [Date] + zVersionNo in (
SELECT [Project Number] + [Contract Number] + [Date] + CONVERT(VARCHAR,(MAX(CAST(zVersionNo AS INT)))) FROM [gpd].[Marine_Spreadrate_PMR_JSON_v]
GROUP BY [Project Number] , [Contract Number] , [Date]);
GO
