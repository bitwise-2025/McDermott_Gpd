/****** Object:  StoredProcedure [dbo].[load_NCSA_fact_MCPM_ProgressItem_validate]    Script Date: 1/5/2023 11:34:16 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROC [dbo].[load_NCSA_fact_MCPM_ProgressItem_validate] AS
BEGIN
...
09/19/2022 ;	Satish		; Change: Changing the Data_sub_class and adding more FACT entries sk/nk
08/12/2022 ;	Aditya		; Change: Addition of Unit, PSRLevel1, PSRLevel2, PSRL1Description, PSRL2Description  columns into stage table
01/04/2023 ;	Abhijit		; Change: Addition of coadesc  column into stage table and replace geaccoa ident2 with coadesc
-----------------------------------------------------------------------------------------------------*/
...
--09/19/2022: Satish -- Adding the Data_sub_class entries since ADF is not configured with parameters correctly
Update [gpd].[NCSA_MCPM_ProgressItems_Onshore]
...
--Please add PSRLevel1 code , Description, PSRLevel2 code , Description to Stage Table and add lines in the Generic Load Details
...
INSERT into gpd.coa_MCPM_PI
	(coa_value,coa_type,project,
	  ...
	  sub_source_name,load_date, PSRLevel1, PSRLevel2, PSRL1Description, PSRL2Description)
select DISTINCT 'JDE.' + '00' + left(ltrim(rtrim(Contract)),3) + '.' + CoA as coa_value,'Project' as coa_type,Domain as project,
	  '' as projectname,
	   ...
	   WorkArea as areaunit,'' as areadesc,costcode,coadesc as costdescription,costtype,'Active' as coa_status,'System' as source_name,
	   'MCPM Onshore' as sub_source_name,getdate() as load_date, PSRLevel1, PSRLevel2, PSRL1Description, PSRL2Description
...
--------------------------------------------------------------------------
--select * from [gpd].[NCSA_MCPM_ProgressItems_Onshore]
		...
		--Step ii: Collect data from MCPM and reprocess to be updated into DIM_ENT_JDE_COA--
				...
				INSERT INTO [gpd].[NCSA_MCPM_dim_ent_jde_coa]
							([CoA],[Domain] ,[MCPM_PSRLevel1] ,[MCPM_PSRLevel2] ,[MCPM_PSRL1Description] ,[MCPM_PSRL2Description] ,[MCPM_CoaDesc],[contractno],[dim_entp_project_nk],[measurable])
				SELECT DISTINCT [CoA],[Domain] ,[MCPM_PSRLevel1] ,[MCPM_PSRLevel2] ,[MCPM_PSRL1Description] ,[MCPM_PSRL2Description] ,[MCPM_CoaDesc],trim([contractno]),[dim_entp_project_nk],[measurable] 
				FROM 
				(SELECT [CoA],[Domain] ,[PSRLevel1] as MCPM_PSRLevel1 ,[PSRLevel2] as MCPM_PSRLevel2, [PSRL1Description] as MCPM_PSRL1Description ,[PSRL2Description] as MCPM_PSRL2Description ,[coadesc] as MCPM_CoaDesc,contract as [contractno],[dim_ent_project_nk] AS dim_entp_project_nk, 'false' as measurable FROM gpd.NCSA_MCPM_ProgressItems_Onshore
				--WHERE [PSRLevel1]<>'' AND PSRLevel2 <> '' AND PSRL1Description <>'' AND [PSRL2Description] <>''
				UNION ALL
				SELECT [CoA],[Domain] ,[PSRLevel1] ,[PSRLevel2] ,[PSRL1Description] ,[PSRL2Description] ,[coadesc],[contract],[dim_ent_project_nk], 'false' as [measurable]  FROM gpd.reprocess_NCSA_MCPM_ProgressItems_Onshore) A

				
		--Step iii : Call the SPU to generic_load_dim_v2 to load the DIM data.--

				-- select * from gpd.MCPM_coa_v  --debugging statement only
				--select * from generic_load_header where load_name
...
---09/19/2022: Satish -- Adding new Data_sub_class sk/nk Lookup updates
...
execute [stage].[fact_lookup_based_on_one_key] 
       ...
       'sub_class_PrevWeeklyEarnedQty','data_sub_class_name','F',null;
...
update  [gpd].[reprocess_NCSA_MCPM_ProgressItems_Onshore]
		...
		--09/19/2022: Satish -- Adding new Data_sub_class nk
