/****** Object:  View [dbo].[fact_pmr_MCPM_Onshore_progress_v]    Script Date: 8/8/2023 3:58:50 PM ******/
DROP VIEW [dbo].[fact_pmr_MCPM_Onshore_progress_v]
GO

/****** Object:  View [dbo].[fact_pmr_MCPM_Onshore_progress_v]    Script Date: 8/8/2023 3:58:58 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [dbo].[fact_pmr_MCPM_Onshore_progress_v] AS select  z.*,f.dim_ent_prime_nk,'JDE-' + convert(varchar(50),z.dim_ent_jde_coa_nk) coa_key  from 
(select 
fact_entp_sk	  
...
,isnull([transaction_quantity],0) [transaction_quantity]
				from [dbo].[fact_ops]
				where 
				dim_entp_sub_source_nk in (select dim_ent_sub_source_nk from [dbo].[dim_ent_sub_source] where sub_source_name in ('MCPM Onshore','Excel_MCPM_Onshore_Cost'))
				AND 
				dim_entp_data_sub_class_nk in (select dim_ent_data_sub_class_nk from [dbo].[dim_ent_data_sub_class] where data_sub_class_name like 'Progress Module%')) a 
