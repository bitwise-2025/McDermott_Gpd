/****** Object:  View [dbo].[dim_entp_work_area_active_v]    Script Date: 12/8/2021 1:22:59 AM ******/
IF	OBJECT_ID('[dbo].[dim_entp_work_area_active_v]','V') IS NOT NULL
DROP VIEW [dbo].[dim_entp_work_area_active_v]
...
CREATE VIEW [dbo].[dim_entp_work_area_active_v]
AS select
...
dim_entp_phase_nk,
dim_entp_project_nk, -- (12/12/21) : Added this column to refer the correct workarea picked per project in TA FACTOPSRows_V.
project_number -- (12/12/21) : Added this column to refer the correct workarea picked per project in TA FACTOPSRows_V.
from dim_entp_work_area
where active = 1;
GO
IF	OBJECT_ID('[dbo].[dim_entp_work_unit_active_v]','V') IS NOT NULL
DROP VIEW [dbo].[dim_entp_work_unit_active_v]
...
CREATE VIEW [dbo].[dim_entp_work_unit_active_v]
AS select 
dim_entp_work_unit_nk,
work_unit_code,
work_unit_name,
dim_entp_project_nk, -- (12/12/21) : Added this column to refer the correct workunit picked per project in TA FACTOPSRows_V.
project_number -- (12/12/21) : Added this column to refer the correct workunit picked per project in TA FACTOPSRows_V.
from dim_entp_work_unit
where active = 1;
GO
/****** Object:  View [dbo].[fact_ops_as_rows_v]    Script Date: 12/8/2021 12:45:07 PM ******/
IF	OBJECT_ID('[dbo].[fact_ops_as_rows_v]','V') IS NOT NULL
...
left outer join [dbo].[dim_ent_prime_active_v] c on b.prime = c.prime_code and b.project = c.project_number   
left outer join [dbo].[dim_ent_prime_active_v] e on d.prime = e.prime_code and d.dim_entp_project_nk = e.dim_entp_project_nk -- (10/01/21): additional join on dim_entp_project_nk
left outer join [dbo].[dim_ent_sub_prime_active_v] f on b.sub_prime = f.sub_prime_code and b.project = f.project_number
left outer join [dbo].[dim_ent_sub_prime_active_v] g on d.sub_prime = g.sub_prime_code and d.dim_entp_project_nk = g.dim_entp_project_nk
) b
where dr = 1
and rn = 1
union all
